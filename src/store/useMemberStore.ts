...
const freshDefaults = () => ({ profile: { ...defaultProfile }, wallet: { ...defaultWallet }, transactions: defaultTransactions.map((item) => ({ ...item })), activities: defaultActivities.map((item) => ({ ...item })), promotions: defaultPromotions.map((item) => ({ ...item })), rewards: defaultRewards.map((item) => ({ ...item })), portalTheme:'neon-night' as PortalTheme })
